...
function interpolate(source, context, noEscape) {
    noEscape = noEscape || false;
    if (typeof source !== 'string') return source;
    if(source.indexOf('{{') === -1) return source;

    var template = handlebars.compile(source, {noEscape: noEscape});
            ...
            (function fn(o) {
                _.keys(o).forEach(function(key) {
                    ...
                    if (typeof o[key] === 'string') {
                        o[key] = encodeURIComponent(interpolate(o[key], context, true));
                    }
                })
            })(cloneContext);

            var result = interpolate(payload, cloneContext, true);
            return result;
        }
